...
#makes a csv file
filename = "metacritic_modern_part_16.csv"
f = open(filename, "w")
...
mc_platforms = ["ps4","xboxone","switch","pc","wii-u","3ds","vita","ios"]
...
for plat in mc_platforms:
	for genre in mc_genres: 	
		#Last Page Grabber
		p_url = 'https://www.metacritic.com/browse/games/genre/metascore/' + genre + '/' + plat + '?view=condensed'
		...
		for page in range(last_page):
			...
			#Slows down web crawler to prevent overlaoding the website 
			if (page % 5) == 0 and page != 0:
				print("CURRENTLY PAUSED, WILL RESUME AFTER 5 MINUTES")
				time.sleep(300)

			#Notifier
			print('Currently at: ' + plat + '-' + genre + ' - ' + 'page: ' + number)

			mc_url = 'https://www.metacritic.com/browse/games/genre/metascore/' + genre + '/' + plat + '?view=condensed&page=' + number

			if page == 0:
				mc_url = 'https://www.metacritic.com/browse/games/genre/metascore/'+ genre + '/'+ plat + '?view=condensed'
			...
			for container in containers:
				...
				if page_empty:
					...
					#Game Title
					if game_page_soup.findAll('a', {"href": link}):
